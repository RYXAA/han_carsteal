...
function SpawnCar()
	ESX.TriggerServerCallback('han_carsteal:isActive', function(isActive, cooldown)
		if cooldown <= 0 then
			if isActive == 0 then
				ESX.TriggerServerCallback('han_carsteal:anycops', function(anycops)
					if anycops >= Config.CopsRequired then
						...
						TriggerServerEvent('han_carsteal:registerActivity', 1)
...
function FinishDelivery()
  if(GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) and GetEntitySpeed(car) < 3 then
    ...
    --Pay the poor fella
		...
		TriggerServerEvent('han_carsteal:pay', finalpayment)

		--Register Activity
		TriggerServerEvent('han_carsteal:registerActivity', 0)
    ...
    TriggerServerEvent('han_carsteal:stopalertcops')
...
Citizen.CreateThread(function()
  while true do
    ...
    if isTaken == 1 and IsPedInAnyVehicle(GetPlayerPed(-1)) then
      ...
      TriggerServerEvent('han_carsteal:alertcops', coords.x, coords.y, coords.z)
...
RegisterNetEvent('han_carsteal:removecopblip')
AddEventHandler('han_carsteal:removecopblip', function()
		RemoveBlip(copblip)
end)

RegisterNetEvent('han_carsteal:setcopblip')
AddEventHandler('han_carsteal:setcopblip', function(cx,cy,cz)
		RemoveBlip(copblip)
...
RegisterNetEvent('han_carsteal:setcopnotification')
AddEventHandler('han_carsteal:setcopnotification', function()
	ESX.ShowNotification(_U('car_stealing_in_progress'))
end)

AddEventHandler('han_carsteal:hasEnteredMarker', function(zone)
...
AddEventHandler('han_carsteal:hasExitedMarker', function(zone)
	CurrentAction = nil
...
Citizen.CreateThread(function()
  while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('han_carsteal:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('han_carsteal:hasExitedMarker', LastZone)
		end
	end
end)
